...
As the outcome and predictors are all quantitative data, it would be not proper to fit the model in Logistic Regression or exponential regression. Thus linear regression may be still the best choice for use to build the model. 
...
```
There were some influential outliers were detected. Further process of outliers are required. 

Let's remove the influential points.
```{r}
bodyfat_out = bodyfat_selected %>%
  filter(!row_number() %in% c(3, 39, 175, 216, 250))
```
...
without1 = lm(bodyfat_brozek ~ age + weight + neck + abdomen + hip + thigh + forearm + wrist, data = bodyfat_out)
...
without2 = lm(bodyfat_brozek ~ age + weight + neck + abdomen + hip + thigh + forearm + wrist + weight*neck + neck*abdomen, data = bodyfat_out)
...
```
Still, even after excluding the influential data, all the predictors, except age, are higly correlated. 
