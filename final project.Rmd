...
```
From the distribution plots of the variables, we can find that, all the variables are in normal distribution with very few outliers. Thus, no transformation is required. There is a sample whose body fat is 0 percent, which is unusual, and we eliminate this point. Since our sample size is small, we won't do any research to remove other unusual date points that are not such obvious.
...
Let's remove the weight term with highest VIF(15.99). 
```{r}
...
Model 2's RMSE(3.945) is slightly smaller than that of model 1(4.009). 
...
```
__bodyfat_brozek = -78.5 + 0.0997*age + 1.33*neck + 1.43*abdomen + 0.0955*thigh + 0.290*forearm -2.02*wrist - 0.0192*neck:abdomen__
